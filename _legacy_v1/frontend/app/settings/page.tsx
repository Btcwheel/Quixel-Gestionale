...
import { Settings, Key, Shield, Bell, Palette, Moon, Sun, Loader2, Plus, Trash2, ToggleLeft, ToggleRight, Smartphone, CheckCircle2, AlertTriangle } from "lucide-react";
...
export default function SettingsPage() {
  ...
  // 2FA state
  const [twoFAStatus, setTwoFAStatus] = useState<string>("disabled");
  const [twoFASecret, setTwoFASecret] = useState<string>("");
  const [twoFAQrUri, setTwoFAQrUri] = useState<string>("");
  ...
  const fetch2FAStatus = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/api/v1/auth/2fa/status`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setTwoFAStatus(res.data.status);
    } catch (error) {
  ...
  const setup2FA = async () => {
    ...
    try {
      ...
      setTwoFAQrUri(res.data.qr_uri);
      setShowTwoFASetup(true);
    } catch (error: any) {
      console.error("Error setting up 2FA:", error);
      alert(error.response?.data?.detail || "Errore nella configurazione 2FA");
    } finally {
  ...
  const verify2FA = async () => {
    ...
    try {
      ...
      setTwoFACode("");
      alert("2FA attivata con successo!");
    } catch (error: any) {
      console.error("Error verifying 2FA:", error);
      alert(error.response?.data?.detail || "Codice 2FA non valido");
    } finally {
      setTwoFALoading(false);
    }
  };
  ...
  const disable2FA = async () => {
    if (!confirm("Sei sicuro di voler disabilitare la 2FA?")) return;
    try {
      const token = localStorage.getItem("access_token");
      await axios.post(
      ...
      alert(error.response?.data?.detail || "Errore nella disattivazione 2FA");
    }
  };
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium">
                      {twoFAStatus === "active" ? "2FA is active" : "2FA is not configured"}
                    </div>
                ...
                {twoFAStatus === "active" ? (
                  ...
                  <Button size="sm" onClick={setup2FA} disabled={twoFALoading}>
                    ...
                    {twoFASecret && (
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                          <code className="flex-1 p-2 bg-muted rounded text-sm font-mono text-center tracking-wider">
                            {twoFASecret}
                          </code>
                          <Button
                            variant="outline"
