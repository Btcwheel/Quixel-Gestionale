...
export async function createAIAccount(formData: FormData) {
  try {
    ...
    console.error('[createAIAccount] error:', err)
    return { error: err instanceof Error ? err.message : 'Impossibile connettersi al database. Verifica che il database sia attivo.' }
  }
}
...
export async function deleteAIAccount(id: string) {
  try {
    ...
    console.error('[deleteAIAccount] error:', err)
    return { error: err instanceof Error ? err.message : 'Impossibile connettersi al database. Verifica che il database sia attivo.' }
  }
}
