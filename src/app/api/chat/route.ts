...
export async function POST(req: Request) {
  try {
    ...
    if (!aiAccount?.api_key_encrypted) {
      return new Response(
        JSON.stringify({ error: 'Nessun account AI collegato a questo progetto. Vai in Impostazioni → aggiungi un account OpenRouter, poi collegalo al progetto.' }),
    ...
    const systemPrompt = `Sei l'assistente AI dedicato al progetto "${project?.name}".${project?.description ? `\nDescrizione: ${project.description}` : ''}
